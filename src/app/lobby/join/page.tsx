...
import { Skull, ArrowRight, Plus, QrCode, LinkIcon } from 'lucide-react';
...
function JoinLobbyContent() {
  ...
  const { joinLobbyOnServer, lastError } = useGameStore();
  ...
  const handleJoinLobby = async () => {
    ...
    const success = await joinLobbyOnServer(lobbyCode.toUpperCase(), playerName.toUpperCase());

    if (success) {
      router.push(`/lobby/${lobbyCode.toUpperCase()}`);
    } else {
      setError(lastError || 'Failed to join lobby. Please check the code and try again.');
      setJoining(false);
    }
  };
