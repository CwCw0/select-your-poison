...
// Generate IDs and combine all strats
function generateId(): string {
  return Math.random().toString(36).substring(2, 9);
}

export const allStrats: Strat[] = [
  ...easyStrats.map(s => ({ ...s, id: generateId() })),
  ...mediumStrats.map(s => ({ ...s, id: generateId() })),
  ...hardStrats.map(s => ({ ...s, id: generateId() })),
  ...stupidStrats.map(s => ({ ...s, id: generateId() })),
];
